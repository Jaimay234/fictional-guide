...
{
    // Define the base address pointer
    // TCSR0 is at offset 0x00, TCRR1 is at offset 0x10 (4 words after)
    unsigned int* timer_ptr = (unsigned int* )XPAR_AXI_TIMER_0_BASEADDR;

    // Stop both timers
    // This is done by clearing ENT (bit 7)
    //*(timer_ptr) &= ~(0x80);        // Stop timer0
    //*(timer_ptr + 4) &= ~(0x80);    // Stop timer1


    // Displays the timer that generated the interrupt and clear TINT bit
    if (*(timer_ptr) & 1<<8) {
        cout << "Interrupt source: Timer0" << endl;

        *(timer_ptr) |= 1<<8;
    }
    if (*(timer_ptr + 4) & 1<<8) {
        cout << "Interrupt source: Timer1" << endl;

        *(timer_ptr + 4) |= 1<<8;
    }

    // Prompt user to enter a value and start both timers
    char input;
    cout << "Press any key to restart both timers..." << endl;
    cin >> input;

    //*(timer_ptr) = 0x0d4;        //Configure timer in generate mode, count up, interrupt enabled with autoreload of load register
    //*(timer_ptr + 4) = 0x0d4;
}
...
int main()
{
 ...
 unsigned int* timer_ptr = (unsigned int* )XPAR_AXI_TIMER_0_BASEADDR;
        ...
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
       *(timer_ptr + 1) = 0x0fff8000;
       *(timer_ptr + 5) = 0x1fff0000;
    ...
    cout << "Enabling the timer to start" << endl;
        ...
        *(timer_ptr) = 0x0d6 ;   // deassert the load 5 to allow the timer to start counting
        *(timer_ptr + 4) = 0x0d6;
